...
import RegistrationForm from "../../../components/components-admin/RegistrationForm";
...
const AdminPokerRoom = () => {
  ...
  const [namePokerRoom, setNamePokerRoom] = useState("");
  ...
  const handleNamePokerRoomChange = (value: string | number) => {
    setNamePokerRoom(String(value));
  };
  const handleShortNameChange = (value: string | number) => {
    setShortName(String(value));
  };
  const handleAdressRoomChange = (value: string | number) => {
  ...
  // HandleModal
  const handleNamePokerRoomChangeModal = (value: string | number) => {
    setNamePokerRoomModal(String(value));
  };
  ...
  const handleAdressRoomChangeModal = (value: string | number) => {
    setAdressModal(String(value));
  };
  ...
  const clickAddPokerTour = async () => {
    ...
    try {
      const response = await pokerRoomApi.createPokerRoom(dataCreate);
      console.log("Thành công", response);
      toast.success("Create poker room successfully");
      defauthValue();
    } catch (error) {
  ...
  const clickUpdatePokerRoom = async () => {
    const dataUpdate: Object = {
      name: namePokerRoomModal,
    ...
    try {
      const res = await pokerRoomApi.updatePokerRoom(
        idModal ? idModal : "",
        dataUpdate
      );
      toast.success("Cập nhật thông tin poker room thành công");
      setShowDetailPokerRoom(false);
    } catch (error) {
      toast.error("Cập nhật poker tour thất bại, vui lòng kiểm tra lại!!!");
    }
  };

  const clickDeletePokerRoom = async (id: string) => {
    try {
      const res = await pokerRoomApi.deletePokerRoom(id);
      toast.success("Xóa poker room thành công");
    } catch (error) {
      toast.error("Xóa poker room thất bại, vui lòng kiểm tra lại!!!");
    }
  };
  ...
  const currentRows: PokerRooms[] = dataPokerRoom.slice(startIndex, endIndex);
  ...
  const demMang = () => {
    for (var i = 0; i < dataPokerRoom.length; i++) {
      totalPokerRooms = i + 1;
    }
  };
  demMang();
  ...
  const fetchData = async () => {
    const res = await pokerRoomApi.getPokerRoom();
    setDataPokerRoom(res.data.data);
  };
  ...
  const handleSelectDetailsPokerTour = (pokerRoom: any) => {
    setShowDetailPokerRoom(true);
    setDescriptionModal(pokerRoom.description);
    setNamePokerRoomModal(pokerRoom.name);
    setSelectedAvatarModal(pokerRoom.avatar);
    setSelectedLogoModal(pokerRoom.logo);
    setShortNameModal(pokerRoom.shortName);
    setAdressModal(pokerRoom.Adress);
    setIdModal(pokerRoom._id);
  };
  ...
  return (
    <div>
      <div className="flex justify-between gap-8 items-center">
        <div className="flex gap-8 justify-center items-center">
          ...
          <p className="font-normal tracking-wide flex gap-1 justify-center items-center text-sm">
            Quantity:{" "}
            <span className="text-blue-400 font-bold">{totalPokerRooms}</span>
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-col justify-start items-center w-[50%]">
            <div className="w-full flex-col flex gap-5">
              ...
              <InputAdmin
                type="text"
                value={namePokerRoom}
                onChange={handleNamePokerRoomChange}
                label="Name Poker Room"
                // validate={(value) => /^[A-Za-z\s]+$/.test(namePlayer)}
                placeholder="Enter poker room name here"
              />
              <InputAdmin
                type="text"
                value={shortName}
              ...
              <InputAdmin
                type="text"
                value={adress}
                onChange={handleAdressRoomChange}
                label="Adress"
                // validate={(value) => /^[A-Za-z\s]+$/.test(namePlayer)}
                placeholder="Enter adress poker room here"
              />
            </div>
          </div>
          <div className="flex flex-col justify-start items-center w-[50%]">
      ...
      <div className="mt-5 rounded-lg bg-white shadow-xl">
        <div className="px-8 py-4 text-left">
          <h1 className="text-2xl font-bold text-left">List Poker Rooms</h1>
        </div>
            ...
            <tbody>
              {currentRows.map((row, index) => (
                ...
                >
                  ...
                  <td className="px-[16px] py-[20px] text-center font-bold text-green-500 min-w-[80px]">
                    {truncateText(row.Adress, 20)}
                  </td>
                  <td className="px-[16px] py-[20px] text-center min-w-[80px]">
                      ...
                      >
                        <div className="bg-[#efefef] rounded-2xl relative shadow-md shadow-gray-300 px-6 py-2 text-left">
                          ...
                          <button
                            ...
                            onClick={() => clickDeletePokerRoom(row._id)}
                          >
                            <FontAwesomeIcon
                              className="text-xl"
                              icon={faTrash}
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center">
          ...
          <div className="bg-white w-[800px] h-auto relative z-30 shadow-xl rounded-lg p-6">
            <div className="absolute right-6 top-6">
              <FontAwesomeIcon
                onClick={() => setShowDetailPokerRoom(false)}
            ...
            <div className="">
              <h1 className="text-3xl font-bold text-blue-500">
                Details Information Poker Room
              </h1>
            </div>
            <div className="px-8 pb-4 mt-8 flex gap-6">
              <div className="flex flex-col justify-start items-center w-[50%]">
                <div className="w-full flex-col flex gap-5">
                  ...
                  <InputAdmin
                    type="text"
                    value={namePokerRoomModal}
                    onChange={handleNamePokerRoomChangeModal}
                  ...
                  <InputAdmin
                    type="text"
                    value={adressModal}
                    onChange={handleAdressRoomChangeModal}
                    label="Adress"
                    // validate={(value) => /^[A-Za-z\s]+$/.test(namePlayer)}
                    placeholder="Enter adress poker room here"
                  />
